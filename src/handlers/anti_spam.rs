...
use regex::Regex;
use serenity::all::{ChannelId, CreateEmbedAuthor, CreateMessage, GetMessages, GuildId, Message, Timestamp, UserId};
...
pub async fn spam_checker(
        ...
        .find(|m| m.author_id == author_id && m.message_content == *message_content)
    {
        ...
        message_tracker.last_mut().ok_or("Failed to get the last message")?
    };

    if message.channel_ids.len() >= 3 {
        mute_spamer(&mut member, ctx, time, new_message).await?;
...
async fn mute_spamer(
    member: &mut serenity::Member,
    ctx: &serenity::Context,
    time: i64,
    new_message: &Message
) -> CommandResult {
    let time = Timestamp::now().unix_timestamp() + time;
